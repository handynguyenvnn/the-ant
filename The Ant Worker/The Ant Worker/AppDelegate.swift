...
let QuandaStoryBoard:UIStoryboard = UIStoryboard.init(name: "Quanda", bundle:nil)
...
class AppDelegate: UIResponder, UIApplicationDelegate,UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        UINavigationBar.appearance().titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.white,NSAttributedString.Key.font:UIFont.init(name: "AvenirNext-Bold", size: 20) ?? ""]
    ...
    func applicationWillTerminate(_ application: UIApplication) {
        // Called when the application is about to terminate. Save data if appropriate. See also appliMainStoryBoarddBackground:.
    }
    ...
    func startTracking(){
        ...
        self.timerTracking = Timer.scheduledTimer(timeInterval: TimeInterval(TIME_TRACKING), target: self, selector: #selector(self.requestTracking), userInfo: nil, repeats: true)
    }
    func stopTracking(){
        if self.timerTracking != nil{
            self.timerTracking?.invalidate()
    ...
    @objc func requestTracking(){
        ...
        Services.shareInstance.trackingLocation(lat: "\(self.currentLocation?.coordinate.latitude ?? 0)", lng: "\(self.currentLocation?.coordinate.longitude ?? 0)") { (response, message, errorCode) in
//            print(message ?? "")
        }
    }
    ...
    @objc func handleNotiEvent(){
        ...
        else
        {
            if userInfo != nil {
                ...
                oid = userInfo?["job_id"].string ?? ""
                notiId = userInfo?["id"].string ?? ""
                let userId = userInfo?["user_id"].string ?? ""
                ...
                if (currentViewController?.isKind(of: TaJobDetailViewController.self) ?? false) && ((currentViewController as! TaJobDetailViewController).jobObj?.internalIdentifier == oid){
                    (currentViewController as! TaJobDetailViewController).initData()
                }
                userInfo = nil
                switch type{
                case .NEW_JOB, .CANCEL_JOB, .JOB_RESOLVED, .JOB_BID, .JOB_COMING_SOON:
                    alert.addAction(UIAlertAction.init(title: "Xem chi tiết", style: .default, handler: { (action) in
                        self.gotoDetailJob(jobId: self.oid)
                        self.getCountNotif()
                    }))
                    break
                case .JOB_RESOLVED_CONFIRM:
                    alert.addAction(UIAlertAction.init(title: "Đồng ý", style: .default, handler: { (action) in
                        self.confirmResolveJob(isAccept: true)
                    }))
                    alert.addAction(UIAlertAction.init(title: "Từ chối", style: .destructive, handler: { (action) in
                        self.confirmResolveJob(isAccept: false)
                    }))
                    break
                case .RE_INVITE:
                    alert.addAction(UIAlertAction.init(title: "Xem danh sách", style: .default, handler: { (action) in
                        self.reInvite()
                        self.getCountNotif()
                    }))
                    break
    ...
    func confirmResolveJob(isAccept:Bool){
    ...
    func gotoDetailJob(jobId:String){
        ...
        if (currentViewController?.isKind(of: TaJobDetailViewController.self) ?? false) && ((currentViewController as! TaJobDetailViewController).jobObj?.internalIdentifier == jobId){
            (currentViewController as! TaJobDetailViewController).getInfoJob()
        }
        else{
    ...
    func reInvite(){
        let listInviteVC:TaListInviteViewController = QuandaStoryBoard.instantiateViewController(withIdentifier: "TaListInviteViewController") as! TaListInviteViewController
        
        self.currentViewController?.navigationController?.pushViewController(listInviteVC, animated: true)
    }
...
extension AppDelegate: CLLocationManagerDelegate {
    ...
    public func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        if status == .authorizedAlways || status == .authorizedWhenInUse {
            ...
            let alert = UIAlertController(title: "Thông báo", message: "Bạn cần bật quyền truy cập vị trí để có thể nhận công việc", preferredStyle: .alert)
            alert.addAction(UIAlertAction.init(title: "Bỏ qua", style: .cancel, handler: { (action) in
                
            }))
            alert.addAction(UIAlertAction.init(title: "Cài đặt", style: .default, handler: { (action) in
