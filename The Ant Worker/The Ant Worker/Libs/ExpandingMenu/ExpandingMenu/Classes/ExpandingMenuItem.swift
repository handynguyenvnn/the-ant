...
open class ExpandingMenuItem: UIView {
    ...
    fileprivate func createTitleButton(_ title: String, titleColor: UIColor? = nil) -> UIButton {
        let button = UIButton()
        button.backgroundColor = UIColor.init(red: 1, green: 1, blue: 1, alpha: 0.9)
        ...
        #endif
        button.titleLabel?.font = UIFont.init(name: "AvenirNext-DemiBold", size: 12)
        button.sizeToFit()
        button.frame = CGRect.init(origin: CGPoint.zero, size: CGSize.init(width: 81, height: 28))
