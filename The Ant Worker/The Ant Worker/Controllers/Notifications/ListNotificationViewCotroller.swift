...
extension ListNotificationViewCotroller: UITableViewDataSource, UITableViewDelegate
{
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        /// từ list nhảy vảo màn list invite
        if TYPE_NOTI.RE_INVITE.rawValue == listNotification[indexPath.row].notificationType{
            ...
            listInviteVC.currentNotifObj = listNotification[indexPath.row]
            self.navigationController?.pushViewController(listInviteVC, animated: true)
        }
        else if let jobId = listNotification[indexPath.row].jid, jobId != ""{
            /// từ list noti nhảy vào màn detail
            self.getNotiDetail(notiId: listNotification[indexPath.row].notificationId ?? "")
            gotoDetailJob(jobId: jobId)
        }
        else{
            /// khi không trả về jobid thì sẽ nhảy vào detail noti
