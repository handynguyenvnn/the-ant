    ...
    override func initData() {
        ...
        if  self.currentNotifObj != nil {
            ...
            Services.shareInstance.getNotifDetail(notifId: self.currentNotifObj?.notificationId ?? notiId ?? "") { (result, message, errorCode) in
                ...
                {
//                    if self.currentNotifObj?.isRead ?? false{
//
//                    }
//                    else{
...
//                    }
        ...
        {
            Toast.init(text: "Không có dữ liệu", delay: 0, duration: 2.0).show()
        }
    }
    ...
    func updateData(){
        if currentNotifObj != nil{
            self.notifiAvatar?.sd_setImage(with: URL.init(string: self.currentNotifObj?.notificationIcon ?? ""), placeholderImage: UIImage.init(named: "ic_non_image-1"), options: .refreshCached, completed: nil)
