    ...
    func tabBarController(_ tabBarController: UITabBarController, shouldSelect viewController: UIViewController) -> Bool {
        if DataCenter.sharedInstance.currentUser?.status == 0{
            if !((viewController as? UINavigationController)?.viewControllers.first?.isKind(of: TAProfileViewController.self) ?? false){
                let alert = UIAlertController(title: "Thông báo", message: "Tài khoản của bạn chưa được kích hoạt nên không thể sử dụng chức năng này, vui lòng liên hệ với The Ant theo số 0999999999.", preferredStyle: .alert)
