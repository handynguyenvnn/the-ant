...
class TaBankInfoViewController: BaseViewController {
    ...
    func postInfoBank(){
        ...
        Services.shareInstance.postBankInfo(bank_name: txtNameBank.text ?? "", account_name: txtFullName.text ?? "", branch_name: txtChiNhanh.text ?? "", bank_code:profile?.bank_code ??  codeBank, bank_short_name:profile?.bank_short_name ?? bankShort, account_number: txtSoTaiKhoan.text ?? "") { (response, message, errorCode) in
    ...
    func fillData(){
        if profile != nil{
            ...
            txtNameBank.text = profile?.bank_name ?? ""
